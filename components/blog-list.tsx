        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {posts.map((post, i) => (
            <Link
              key={post.slug}
              href={`/blog/${post.slug}`}
              className={`${scrollRevealClass(isInView, Math.min(i, 6) as ScrollRevealDelay)} flex flex-col gap-3 rounded-lg border border-border bg-card p-6 transition-all duration-200 ease-out hover:-translate-y-1 hover:bg-secondary hover:shadow-[0_0_24px_rgba(143,56,72,0.4)] active:scale-[0.99] active:opacity-90`}
            >
